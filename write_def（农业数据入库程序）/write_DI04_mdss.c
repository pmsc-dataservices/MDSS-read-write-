	...
	//printf("fengyuxing6\n");
    	...
    	if(num==COLUMN_NUM)
    	{
          //printf("fengyuxing7\n");
    		...
    		*/
    		sprintf(date,"%04d",atoi(temp[3]));
    		sprintf(date+4,"%02d",atoi(temp[4]));
    		sprintf(date+6,"%02d",atoi(temp[5]));
                sprintf(date+14,"%06d",atoi(temp[6]));
    		printf("%s\n",date);
    		sprintf(cymd,"%08d",atoi(temp[0]));
    		printf("%s\n",cymd);
                sprintf(chms,"%06d",atoi(temp[1]));
    		printf("%s\n",chms);
                sprintf(des,"%s",temp[2]);
                printf("%s\n",des);


    		for(j=3;j<COLUMN_NUM;j++)
    	  {
    		  //printf("%f ",atof(temp[j]));
    		  value[j-3]=atof(temp[j]);
    		  //printf("%f ",value[j]);
    		}
    		//printf("fengyuxing8\n");
    		ret=insert_DI04(date,cymd,chms,des,value);
                //printf("fengyuxing9\n");

    		if(ret!=0)
    		{
    			printf("call insert_DI04() failed.\n");
    			exit(-1);
    		}
    		record++;
    		//printf("\n");
