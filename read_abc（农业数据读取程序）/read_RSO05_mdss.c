...
#define  FDATANUM  13000000   //从数据库中返回的整型数据总数：record_num*column_float_num。abwa有32个实型字段：32×400000=12800000
#define  CDATANUM  400000   ////从数据库中返回的字符型数据总数：record_num*column_char_num。abwa没有字符型数据
#define  IDENNUM   400000   //long ciden[],fiden[]数组大小：从数据库中返回的记录总数。一个站一年有12*3条记录。假设有10000个站：12×3×10000=360000
...
/*--- 4. 设置输出文件  ---*/
    sprintf(output, "%s/data/Z_data/SO05_%s_%s.dat", getenv("HOME"),argv[1],argv[2]);
      
       ret = elemsea(1104, cpar, lpar, cdata, fdata, ciden, fiden);
