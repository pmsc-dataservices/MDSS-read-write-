...
int main(int argc,char *argv[]) 
{
    ...
    sprintf(logname, "%s/log/product_gwsp_log/gwsp_%04d%02d%02d.log", getenv("HOME"),ct[0],ct[1],ct[2]);
...
/*--- 4. 设置输出文件  ---*/
    sprintf(output,  "%s/data/rgwsp/rgwsp_%s%s_005.txt", getenv("HOME"),argv[1],argv[4]);
...
int OutPut(char filename[],long fiden[],long ciden[],float fdata[],char cdata[][CF_LEN])
{
    ...
    {
      	fprintf(fp,"\n");
      fprintf(fp," %.1f  %.1f  %.4f  %.4f  %.4f ",fdata[0+fdata_num],fdata[1+fdata_num],fdata[2+fdata_num],fdata[3+fdata_num],fdata[4+fdata_num]);

 			for (j=5; j<fiden[i+4]; j++) //每条记录实型数据总个数 
 			{
	    	if(j%141==0 && j!=4)
	    	fprintf(fp,"\n");
      fprintf(fp," %.1f ",fdata[j+fdata_num]);
