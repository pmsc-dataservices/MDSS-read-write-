...
int main(int argc, char *argv[])
{
   ...
   char user[30], passwd[30], dbname[30];
    char itime[14],ptime[14],sdir[300],full_filename[300],tdir1[300],tdir2[300],tdir3[300],fname[500],filesize[50],pname[50],sip[20],sname[50],tip1[20],tip2[20],tip3[20],tname1[50],tname2[50],tname3[50];
    char data_dir[50],data_file[50],data_file1[50],data_file2[50];
    FILE *file_list;
     FILE *file_list1;
     FILE *file_list2;
	  ...
	  char cmd[1000],cmd1[1000],cmd2[1000],cmd3[1000], argc_radarno[10];
    ...
    sprintf(data_file, "%s/bin/ftp_radar_240/240_data_%s.sh", getenv("HOME"), argv[1]);
     sprintf(data_file1, "%s/bin/ftp_radar_122/122_data_%s.sh", getenv("HOME"), argv[1]);
     sprintf(data_file2, "%s/bin/ftp_radar_233/233_data_%s.sh", getenv("HOME"), argv[1]);
    ...
    if ((file_list1=fopen(data_file1,"w")) == NULL) 
    {
        ...
        fprintf(file_list1,"source /home/radar_user/.bash_profile\n");             
    	   fprintf(file_list1,"FTP_PRODUCT_FILE=/home/radar_user/log/ftp_radar_122_%s.log\n",argv[1]);  
     ...
     if ((file_list2=fopen(data_file2,"w")) == NULL) 
    {
         fprintf(fplog, "cannot write file %s\n", data_file1);
         return -1;
    }
        fprintf(file_list2,"source /home/radar_user/.bash_profile\n");             
    	   fprintf(file_list2,"FTP_PRODUCT_FILE=/home/radar_user/log/ftp_radar_233_%s.log\n",argv[1]);  
        fprintf(file_list2,"echo \"user radarbase  radarbase123\" > $FTP_PRODUCT_FILE\n");  
         fprintf(file_list2,"echo \"bin\" >> $FTP_PRODUCT_FILE\n");    
         fprintf(file_list2,"echo \"prom\" >> $FTP_PRODUCT_FILE\n");    
    ...
    while(1)
    {
       ...
       for(i=0; i<file_num; i++)
       {
          ...
          fprintf(file_list1,"echo \"lcd /home/radar_user/radardata/%s\" >> $FTP_PRODUCT_FILE\n",radar_number);       
           fprintf(file_list1,"echo \"mput Z_RADR_I_%s_%s_%s_%s_%s_CAP.bin\" >> $FTP_PRODUCT_FILE\n",str[3],str[4],str[5],str[6],str[7]);   
             ...
             fprintf(file_list2,"echo \"mkdir /%s\" >> $FTP_PRODUCT_FILE\n",radar_number);     
           fprintf(file_list2,"echo \"cd /%s\" >> $FTP_PRODUCT_FILE\n",radar_number); 
          fprintf(file_list2,"echo \"lcd /home/radar_user/radardata/%s\" >> $FTP_PRODUCT_FILE\n",radar_number);       
           fprintf(file_list2,"echo \"mput Z_RADR_I_%s_%s_%s_%s_%s_CAP.bin\" >> $FTP_PRODUCT_FILE\n",str[3],str[4],str[5],str[6],str[7]);   
            ...
            sprintf(tname1, "nmic_provider");
             // sprintf(tip2, "10.30.16.122");
              sprintf(tip2, "10.14.83.121");
       ...
       fprintf(file_list,"ftp -nv 10.30.16.240< $FTP_PRODUCT_FILE\n\n");  
         fprintf(file_list1,"ftp -nv 10.14.83.121< $FTP_PRODUCT_FILE\n\n");  
          fprintf(file_list2,"ftp -nv 10.30.16.233< $FTP_PRODUCT_FILE\n\n");  
    ...
    fclose(fplog);
     ...
     fclose(file_list);
      ...
      fclose(file_list2);
          ...
          sprintf(cmd2, "sh %s/bin/ftp_radar_122/122_data_%s.sh", getenv("HOME"), argv[1]);
          system(cmd2); 

            memset(cmd3, '\0', sizeof(cmd2));
          sprintf(cmd3, "sh %s/bin/ftp_radar_233/233_data_%s.sh", getenv("HOME"), argv[1]);
          system(cmd3); 
